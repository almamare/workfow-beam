...
import { Trash2, SquarePen, Eye, Plus, RefreshCw, FileSpreadsheet, Download } from 'lucide-react';
...
export default function ContractorsPage() {
    ...
    const handleDownload = async (row: Contractor) => {
        setDownloadingId(row.id);
        try {
            const res = await axios.get(`/contractors/download/${row.id}`, { responseType: 'blob' });
            const url = window.URL.createObjectURL(new Blob([res.data], { type: 'application/pdf' }));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `contractor_${row.number || row.id}.pdf`);
            document.body.appendChild(link);
            link.click();
            link.remove();
            window.URL.revokeObjectURL(url);
    ...
    const actions: Action<Contractor>[] = [
        {
            ...
            onClick: (row: Contractor) => router.push(`/contracts/contractors/details?id=${row.id}`),
        ...
        },
        {
            ...
            onClick: (row: Contractor) => router.push(`/contracts/contractors/update?id=${row.id}`),
            variant: 'warning' as const,
            hidden: (row: Contractor) => row.status === 'Active'
        },
        {
        ...
        <>
            <div className="space-y-4">
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-2 md:space-y-0">
                    ...
                    <Button
                        onClick={() => router.push('/contracts/contractors/create')}
                    ...
                    headerActions={
                        ...
                        >
                            ...
                            <SelectContent className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 shadow-lg">
                                ...
                                <SelectItem value="200" className="text-slate-900 dark:text-slate-100 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-brand-sky-600 dark:hover:text-brand-sky-400 focus:bg-slate-100 dark:focus:bg-slate-700 focus:text-brand-sky-600 dark:focus:text-brand-sky-400 cursor-pointer transition-colors duration-200">200 per page</SelectItem>
                            </SelectContent>
                        </Select>
                    }
                >
