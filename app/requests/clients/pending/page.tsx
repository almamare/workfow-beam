...
import {
    fetchClientsRequestsForApproval,
    setFilters,
    setPage,
    resetFilters,
    selectClientsRequestsForApproval,
    selectClientsRequestsForApprovalTotal,
    selectClientsRequestsForApprovalPages,
    selectClientsRequestsForApprovalCurrentPage,
    selectClientsRequestsForApprovalLoading,
    selectClientsRequestsForApprovalError,
    selectClientsRequestsForApprovalFilters,
} from '@/stores/slices/clients_requests_for_approval';
...
import { Building, Eye, CheckCircle, XCircle, RefreshCw, FileSpreadsheet, Clock, Search, X, FileText } from 'lucide-react';
...
// Extended ClientRequest with client data
interface ClientRequestWithData extends ClientRequest {
    client?: Client;
    current_step_level?: string | number; // Current approval step level
}

function ClientsPendingPageContent() {
    const items = useSelector(selectClientsRequestsForApproval);
    const total = useSelector(selectClientsRequestsForApprovalTotal);
    const pages = useSelector(selectClientsRequestsForApprovalPages);
    const currentPage = useSelector(selectClientsRequestsForApprovalCurrentPage);
    const loading = useSelector(selectClientsRequestsForApprovalLoading);
    const error = useSelector(selectClientsRequestsForApprovalError);
    const filters = useSelector(selectClientsRequestsForApprovalFilters);
    ...
    // Modal state
    const [selectedRequest, setSelectedRequest] = useState<ClientRequestWithData | null>(null);
    ...
    useEffect(() => {
        ...
        dispatch(fetchClientsRequestsForApproval(params));
    ...
    useEffect(() => {
        items.forEach(request => {
            // For client requests, client_id is stored in the request
            const clientId = (request as any).client_id || (request as any).task_order_id;
            if (clientId && !clientsData.has(clientId)) {
                fetchClientData(clientId);
            }
        });
    }, [items, fetchClientData, clientsData]);

    // Create enriched requests with client data
    const enrichedRequests = useMemo(() => {
        return items.map(request => {
            const clientId = (request as any).client_id || (request as any).task_order_id;
            const client = clientId ? clientsData.get(clientId) : undefined;
            return {
                ...request,
                client
    ...
    const refreshTable = async () => {
        ...
        try {
            await dispatch(fetchClientsRequestsForApproval({
                page: currentPage,
                limit,
        ...
        try {
            ...
            const exportData = await Promise.all(
                items.map(async (req: any) => {
                    ...
                    const clientId = req.client_id || req.task_order_id;
                    if (clientId) {
                        try {
                            const result = await dispatch(fetchClient(clientId)).unwrap();
                            if (result?.data?.client) {
            ...
            a.href = url;
            a.download = `client_pending_requests_${new Date().toISOString().slice(0,10)}.csv`;
            a.click();
    ...
    const handleModalClose = () => {
        ...
        dispatch(fetchClientsRequestsForApproval({
            page: currentPage,
            limit,
            search: debouncedSearch,
            from_date: dateFrom || undefined,
    ...
    const columns: Column<ClientRequestWithData>[] = useMemo(() => [
        ...
        },
        {
            ...
            render: (value: any, row: ClientRequestWithData) => {
                ...
                const clientId = (row as any).client_id || (row as any).task_order_id;
                const isLoading = loadingClientsRef.current.has(clientId);
                if (isLoading) {
        ...
        {
            key: 'step_name' as keyof ClientRequestWithData,
            header: 'Step Name',
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-2 md:space-y-0">
                <div>
                    <h1 className="text-3xl md:text-4xl font-bold text-slate-800 dark:text-slate-200">Client Requests - Pending Approval</h1>
                    <p className="text-slate-600 dark:text-slate-400 mt-1">
                        Browse and manage all pending client requests that require your approval
                    </p>
                </div>
            </div>
            ...
            <EnhancedCard
                ...
                description="Search and filter pending client requests by various criteria"
                variant="default"
                size="sm"
            >
                <div className="space-y-4">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                {activeFilters.map((filter, index) => (
                                    <Badge
                                        key={index}
                                        variant="outline"
                                        className="bg-brand-sky-100 dark:bg-brand-sky-900/30 text-brand-sky-700 dark:text-brand-sky-300 hover:bg-brand-sky-200 dark:hover:bg-brand-sky-900/50 border-brand-sky-200 dark:border-brand-sky-800"
                                    >
                                        {filter}
                                    </Badge>
                                ))}
            ...
            {/* Requests Table */}
            <EnhancedCard
                title="Pending Client Requests List"
                description={`${total} pending request${total !== 1 ? 's' : ''} found`}
                variant="default"
                size="sm"
                stats={{
                    total: total,
                    badge: 'Total Pending Requests',
                    badgeColor: 'default'
                }}
                headerActions={
                ...
                {error ? (
                    <div className="flex items-center justify-center py-12">
                        <div className="text-center">
                            ...
                            <Button
                                variant="outline"
                                onClick={() => dispatch(fetchClientsRequestsForApproval({
                                    page: currentPage,
                                    limit,
                                    search: debouncedSearch,
                                    from_date: dateFrom || undefined,
...
export default function ClientsPendingPage() {
    return <ClientsPendingPageContent />;
}
