...
/* =========================================================
   Main Component
=========================================================== */
function ProjectContractRequestDetails() {
    ...
    const contract = useAppSelector(selectSelectedProjectContract) as ProjectContract | null;
    const contractLoading = useAppSelector(selectProjectContractsLoading);
    ...
    useEffect(() => {
        if (!id) {
            ...
            router.push('/requests/contracts/contracts');
            return;
        }
    ...
    // Fetch project contract (task_order_id = contract_id for this request type)
    useEffect(() => {
        if (request?.task_order_id) {
            dispatch(fetchProjectContract(request.task_order_id));
        }
        return () => {
    ...
    // Combined loading
    const loading = requestLoading || contractLoading || isLoading;
    ...
    if (requestError) {
        return (
            <Centered>
                ...
                <Button
                    variant="outline"
                    onClick={() => router.push('/requests/contracts/contracts')}
    ...
    if (!request) {
        return (
            <Centered>
                <p className="text-rose-600 dark:text-rose-400">Project contract request not found</p>
                <Button
                    variant="outline"
                    onClick={() => router.push('/requests/contracts/contracts')}
                    className="border-brand-sky-200 dark:border-brand-sky-800 hover:text-brand-sky-700 hover:border-brand-sky-300 dark:hover:border-brand-sky-700 text-brand-sky-700 dark:text-brand-sky-300 hover:bg-brand-sky-50 dark:hover:bg-brand-sky-900/20"
            ...
            <div className="flex items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-600 dark:text-slate-400 mt-2">
                        A brief overview of the project contract approval request with available actions.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                >
                    <div className="text-xl md:text-lg font-bold text-slate-900 dark:text-slate-100">
                        {request.request_type || 'ProjectContracts'}
                    </div>
                </EnhancedCard>
                <EnhancedCard
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4">
                ...
                {contract && (
                    <EnhancedCard
                        ...
                        description="Commercial terms linked to this approval"
                        variant="default"
                        size="sm"
                        headerActions={
                            <Button
                                variant="outline"
                                onClick={() => router.push(`/contracts/contracts/details?id=${contract.id}`)}
                                className="border-brand-sky-200 dark:border-brand-sky-800 hover:text-brand-sky-700 hover:border-brand-sky-300 dark:hover:border-brand-sky-700 text-brand-sky-700 dark:text-brand-sky-300 hover:bg-brand-sky-50 dark:hover:bg-brand-sky-900/20"
                            >
                    ...
                    >
                        <div className="divide-y divide-slate-200 dark:divide-slate-800 space-y-1">
                            <Detail label="Contract No." value={contract.contract_no} />
                            ...
                            <Detail
                                ...
                                value={
                                    contract.approval_status ? (
                                        <Badge variant="outline" className={getStatusColor(contract.approval_status)}>
                                            {contract.approval_status}
                                        </Badge>
                                    ) : 'N/A'
                                }
                            ...
                            <Detail
                                label="Value"
                                value={contract.contract_value != null ? `${contract.contract_value} ${contract.currency || ''}` : 'N/A'}
                            />
                            {contract.project_name && <Detail label="Project" value={contract.project_name} />}
                        </div>
                    </EnhancedCard>
                )}
                ...
                >
                    <div className="divide-y divide-slate-200 dark:divide-slate-800 space-y-1">
                        ...
                        <Detail label="Request Type" value={request.request_type || 'ProjectContracts'} />
