...
function TasksApprovedPageContent() {
    ...
    // Filter task requests to only show "Tasks" type with "Approved" status
    const taskRequestsFiltered = useMemo(() => {
        return taskRequests.filter(req => req.request_type === 'Tasks' && req.status === 'Approved');
    }, [taskRequests]);
    ...
    // Fetch task requests with request_type = "Tasks" and status = "Approved"
    useEffect(() => {
    ...
    const exportToExcel = async () => {
        ...
        try {
            ...
            a.href = url;
            a.download = `approved_task_requests_${new Date().toISOString().slice(0, 10)}.csv`;
            a.click();
    ...
    const columns: Column<TaskRequest>[] = useMemo(() => [
        ...
        },
        {
            ...
            header: 'Request Type',
            render: (value: any) => (
                <Badge variant="outline" className="bg-brand-sky-100 dark:bg-brand-sky-900/30 text-brand-sky-700 dark:text-brand-sky-300 border-brand-sky-200 dark:border-brand-sky-800">
                    {value || 'Tasks'}
                </Badge>
            ),
            sortable: true
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-2 md:space-y-0">
                <div>
                    <h1 className="text-3xl md:text-4xl font-bold text-slate-800 dark:text-slate-200">Approved Task Requests</h1>
                    <p className="text-slate-600 dark:text-slate-400 mt-1">
                        Browse and manage all approved task requests
                    </p>
                </div>
            </div>
            ...
            <EnhancedCard
                ...
                description="Search and filter approved task requests"
                variant="default"
                size="sm"
            >
                <div className="space-y-4">
            ...
            {/* Requests Table */}
            <EnhancedCard
                title="Approved Task Requests List"
            ...
            >
                <EnhancedDataTable
                    ...
                    noDataMessage="No approved task requests found matching your criteria"
